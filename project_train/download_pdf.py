    ...
    def __init__(self, page_num):
        ...
        while True:
            try:
                ...
                self.response = requests.post(self.source_info_url, data=self.post_args, timeout=(5, 60 * 10))
                break
            except:
                try_times += 1
                if try_times > 3:
    ...
    def __download_one_company_pdf(self, index):
        ...
        for pdf_index in range(wxhf_num):
            ...
            if not os.path.exists(save_path):
                with closing(requests.get(file_url_path, stream=True, timeout=(5, 60 * 10))) as response:
                    with open(save_path, "wb") as f:
                        # chunk_size 512 bytes
                        for chunk in response.iter_content(chunk_size=512):
                            if chunk:
                                f.write(chunk)
                print('第--{}--页,第--{}--个公司的第--{}个/{}--pdf文件<{}>下载完毕'.format(self.page_num, index, pdf_index, wxhf_num,
                                                                            file_title))
            else:
